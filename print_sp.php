...
$stmt2 = $conn->prepare("SELECT tblperingatan.id_peringatan, tblperingatan.idpeg, tblperingatan.ditetapkan, tblperingatan.tanggal, tblperingatan.alasan, tblperingatan.pembuat_surat, tabelpegawai.nama 
                         FROM tblperingatan
                         JOIN tabelpegawai ON tblperingatan.idpeg = tabelpegawai.idpeg
                         WHERE tblperingatan.id_peringatan = ? LIMIT 1");
$stmt2->bind_param('s', $id);
$stmt2->execute();
$stmt2->bind_result($id_peringatan, $idpeg, $ditetapkan, $tanggal, $alasan, $pembuat_surat, $namaPegawai);
$stmt2->fetch();
$stmt2->close();
...
$pdf->Cell(0, 10, 'Surat Peringatan', 0, 1, 'C');
$pdf->SetDrawColor(0, 0, 0); // Warna hitam
...
$pdf->Cell(0, 10, 'Nomor : ' . $id_peringatan, 0, 1, 'C');
$pdf->Ln(1);

// Isi surat
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(0, 10, 'Surat peringatan ini ditujukan kepada :', 0, 1, 'L');
$pdf->Ln(1);
