...
$result = $conn->query("SELECT p.id_izin, p.tanggal, p.jam, p.alasan, p.idpeg, g.nama, p.ditetapkan, p.pembuat_surat FROM tblizin p JOIN tabelpegawai g ON p.idpeg = g.idpeg ORDER BY p.id_izin DESC");

// Dapatkan nomor urut terbaru untuk id_izin baru
$stmt = $conn->query("SELECT id_izin FROM tblizin ORDER BY id_izin DESC LIMIT 1");
...
$newid = 'IZ' . date('Y') . date('m') . str_pad($urut, 3, '0', STR_PAD_LEFT);
                        ...
                        <table id="izinTable" class="table table-bordered table-striped table-hover">
                            ...
                            <tbody>
                                ...
                                if ($result && $result->num_rows > 0) {
                                    $no = 1;
                                    while ($tblizin = $result->fetch_assoc()) {
                                        echo "<tr>";
                                        echo "<td class='text-center'>" . $no++ . "</td>";
                                        echo "<td class='text-center'>" . htmlspecialchars($tblizin['id_izin']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['idpeg']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['nama']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['tanggal']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['jam']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['alasan']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['ditetapkan']) . "</td>";
                                        echo "<td>" . htmlspecialchars($tblizin['pembuat_surat']) . "</td>";
                                        echo "<td class='text-center'>";
                                        echo "<div class='d-flex justify-content-center'>";
                                        echo "<button class='btn btn-warning btn-sm edit-btn mr-1' data-bs-toggle='modal' data-bs-target='#editizinModal' data-id='" . htmlspecialchars($tblizin['id_izin']) . "' data-tanggal='" . htmlspecialchars($tblizin['tanggal']) . "' data-jam='" . htmlspecialchars($tblizin['jam']) . "' data-alasan='" . htmlspecialchars($tblizin['alasan']) . "' data-ditetapkan='" . htmlspecialchars($tblizin['ditetapkan']) . "' data-pembuat_surat='" . htmlspecialchars($tblizin['pembuat_surat']) ."'><i class='fas fa-edit'></i> Edit</button>";
                                        echo "<button class='btn btn-danger btn-sm delete-btn mr-1' data-id='" . htmlspecialchars($tblizin['id_izin']) . "'><i class='fas fa-trash'></i> Delete</button>";
                                        echo "<button class='btn btn-success btn-sm print-btn' data-id='" . htmlspecialchars($tblizin['id_izin']) . "'><i class='fas fa-print'></i> Print</button>";
                                        echo "</div>";
                                        echo "</td>";
                                        echo "</tr>";
                                    }
