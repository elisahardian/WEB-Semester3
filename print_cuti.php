...
$stmt2 = $conn->prepare("SELECT tabelcuti.id_cuti, tabelcuti.idpeg, tabelcuti.ditetapkan, tabelcuti.tanggal, tabelcuti.alasan, tabelcuti.pembuat_surat, tabelpegawai.nama 
                         FROM tabelcuti
                         JOIN tabelpegawai ON tabelcuti.idpeg = tabelpegawai.idpeg
                         WHERE tabelcuti.id_cuti = ? LIMIT 1");
$stmt2->bind_param('s', $id);
$stmt2->execute();
$stmt2->bind_result($id_cuti, $idpeg, $ditetapkan, $tanggal, $alasan, $pembuat_surat, $namaPegawai);
$stmt2->fetch();
$stmt2->close();
...
$pdf->Cell(0, 10, 'Surat cuti', 0, 1, 'C');
$pdf->SetDrawColor(0, 0, 0); // Warna hitam
...
$pdf->Cell(0, 10, 'Nomor : ' . $id_cuti, 0, 1, 'C');
$pdf->Ln(1);

// Isi surat
$pdf->SetFont('Arial', 'B', 11);
$pdf->Cell(0, 10, 'Surat cuti ini ditujukan kepada :', 0, 1, 'L');
$pdf->Ln(1);
...
// Alasan cuti dengan pengaturan spasi 1.25
$pdf->SetFont('Arial', '', 11);
