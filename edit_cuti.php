...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id_cuti && $idpeg && $tanggal && $alasan && $ditetapkan && $pembuat_surat) {
        ...
        $stmt = $conn->prepare("UPDATE tabelcuti SET idpeg = ?, tanggal = ?, alasan = ?, ditetapkan = ?, pembuat_surat = ? WHERE id_cuti = ?");
        $stmt->bind_param("ssssss", $idpeg, $tanggal, $alasan, $ditetapkan, $pembuat_surat, $id_cuti);
